...
erstellt er ein Dokument mit folgenden Feldern: \texttt{name: <name>} und \texttt{data: []}\footnote{\texttt{[]} ist die notation für ein leeres Array in vielen Programmiersprachen}. Siehe Abbildung~\ref{abb:document}
...
\end{minted}
\caption{Dokument in Datenbank\label{abb:document}}
\end{figure}

% Datenübertagung {{{3
\subsubsection{Übertragung des Gewichts}
Zum Übertragen der Gewichtsdaten macht die Waage eine \texttt{POST} Anfrage an die URL \texttt{/add}, an die das aktuelle Gewicht des Bienenstockes und die bei der Initialisierung erhaltene DokementID im \texttt{JSON} Format angeheftet sind. Siehe Abbildung~\ref{abb:/add}.
...
\end{minted}
\caption{Beispiel einer \texttt{POST} Request auf \texttt{/add}}\label{abb:/add}
\end{figure}

%- server from db document with _id
% server adds

Sobald der Server die Daten der Waage bekommen hat, holt er sich das Dokument mit der \texttt{\_id} aus der Anfrage aus der Datenbank und fügt ein neues Objekt mit dem Gewicht und der aktuellen Zeit (Abbildung~\ref{abb:objTimeWeight}) zum Dataarray des Dokumentes hinzu.
...
\end{minted}
\caption{Objekt mit Zeit und Gewicht in \texttt{JSON}\label{abb:objTimeWeight}}
\end{figure}
